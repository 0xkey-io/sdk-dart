...
class DashboardScreenState extends State<DashboardScreen> {
  ...
  late final ZeroXKeyProvider _zeroxkeyProvider;
  ...
  void initState() {
    ...
    _zeroxkeyProvider = Provider.of<ZeroXKeyProvider>(context, listen: false);

    _providerListener = _handleProviderUpdate;
    _zeroxkeyProvider.addListener(_providerListener);

    // Initialize local selections from provider.
    _updateSelectedWalletFromProvider(_zeroxkeyProvider);
  }

  @override
  void dispose() {
    // Always remove listeners to avoid calling setState on a disposed widget.
    _zeroxkeyProvider.removeListener(_providerListener);
    super.dispose();
  }
  ...
  void _updateSelectedWalletFromProvider(ZeroXKeyProvider provider) {
  ...
  Future<void> handleSign(
    ...
    try {
      final response = await _zeroxkeyProvider.signMessage(
  ...
  Future<void> handleExportWallet(BuildContext context, Wallet wallet) async {
    try {
      final export = await _zeroxkeyProvider.exportWallet(
        walletId: wallet.id,
      );
  ...
  Widget build(BuildContext context) {
    const signMessage = 'I love ZeroXKey';
    ...
    return Scaffold(
      appBar: AppBar(
        leading: IconButton(
          onPressed: () {
            ...
            _zeroxkeyProvider.clearAllSessions();
          },
      ...
      body: Center(
        child: Consumer<ZeroXKeyProvider>(
          builder: (context, zeroxkeyProvider, child) {
            ...
            return Padding(
              ...
              child: Column(
                ...
                children: <Widget>[
                  Text(
                    'Welcome, ${zeroxkeyProvider.user?.userName ?? 'User'}!',
                              ...
                              children: [
                                PopupMenuButton<Wallet>(
                                  ...
                                  itemBuilder: (BuildContext context) {
                                    final wallets = zeroxkeyProvider.wallets;
...
class AddWalletDialogState extends State<AddWalletDialog> {
  ...
  Future<void> handleAddWallet(BuildContext context) async {
    ...
    if (_generateSeedPhrase) {
      await zeroxkeyProvider.createWallet(
      ...
      await zeroxkeyProvider.importWallet(
